...
<form action="reserve.php" method="post" style="display:flex;flex-direction:column;align-items:space-around;">
...
if($_SERVER['REQUEST_METHOD']=="POST"){
    ...
    $stmt = $db->prepare("INSERT INTO tickets (firstname, lastname, dob, outboundairport, inboundairport, flightdate)
    VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param('ssssss', $firstname, $lastname, $dob, $inbound, $outbound, $date);
    $stmt->execute();
    if( $stmt->affected_rows > 0)
    printf("%s, \n Thanks for reserving your flight ticket from %s to %s.", $firstname, $inbound, $outbound);
    $stmt->free_result();
    $db->close();
}
?>
